...
hw5::hw5(string file, string user, string log)
{
   fileName = file;
   userName = user;
   logFileName = log;

}//end hw5(string, string, string) 

//setters
void hw5::setFile(string f)
{
   fileName = f;
}//end setFile()

void hw5::setUser(string u)
{
   userName = u;
}//end setUser()

void hw5::setLog(string l)
{
   logFileName = l; 
}//end setLog()

void hw5::setHomeDir(char * path)
{
   if(strlen(path) > 0)
	homeDir = path;
}//setHomeDir(string)

void hw5::setPort(string port)
{
	portNumber = port;
}//setPort()

void hw5::setServer(string server)
{
	serverName = server;
}//setServer()
...
int hw5::processArgs(int argc, char** argv)
{
   char opt;
   ...
   while( ( opt = getopt(argc, argv, "u:f:l:p:s:")) != -1)
   {
      switch (opt)
      {
        case 'u':
   ...
   //check for user name, port, or server not specified at command line
   if(getUser().length() == 0 || getPort().length() == 0 || getServer().length() == 0)
		return -1;   
...
void hw5::writeLnToLog(string str)
{
   logFile << str << endl;
}//writeLnToLog

//processes commands using a switch on cmd
int hw5::processCmd(char cmd, string param)
{
   int err;
   char c = cmd;
   string p = param;
...
void hw5::changeDir(string param)
{
    string s = param;
    char cwd[256];
    int val = validate(s);
